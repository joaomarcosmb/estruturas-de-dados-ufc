...
class Banco:
    ...
    def ordernar_clientes_por_credito(self, asc=False):
        ...
        while trocado:
            ...
            while atual and atual.proximo:
                deve_trocar = (asc and float(atual.credito) > float(atual.proximo.credito)) or (
                            not asc and float(atual.credito) < float(atual.proximo.credito))
    ...
    def excluir_cliente(self, conta):
        cliente = self.buscar_cliente_por_conta(conta)
        ...
        if cliente.anterior:
            ...
            self.head = cliente.proximo

        if cliente.proximo:
            cliente.proximo.anterior = cliente.anterior
        else:
            self.tail = cliente.anterior
...
def main():
    ...
    while True:
        ...
        if opcao == 1:
            ...
            conta = input("Digite o número da conta do cliente a ser excluído: ")
            banco.excluir_cliente(conta)
        elif opcao == 5:
